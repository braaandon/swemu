...
func init() {
	u, err := user.Current()
	userAndHostname = "fake Let's Encrypt"
}
...
func (m *mkcert) makeCert(hosts []string) {
	...
	tpl := &x509.Certificate{
		...
		Subject: pkix.Name{
			Organization:       []string{"mkcert fake Let's Encrypt"},
			OrganizationalUnit: []string{"fake Let's Encrypt"},
		},
...
func (m *mkcert) newCA() {
	...
	tpl := &x509.Certificate{
		SerialNumber: randomSerialNumber(),
		Subject: pkix.Name{
			Organization:       []string{"fake Let's Encrypt"},
			OrganizationalUnit: []string{"fake Let's Encrypt"},

			// The CommonName is required by iOS to show the certificate in the
			// "Certificate Trust Settings" menu.
			// https://github.com/FiloSottile/mkcert/issues/47
			CommonName: "Fake Let's Encrypt",
		},
		SubjectKeyId: skid[:],
